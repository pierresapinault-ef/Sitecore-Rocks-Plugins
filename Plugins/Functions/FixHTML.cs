...
{
    [Function("FixHTML", Example = "update set @field = FixHTML(@text) from /sitecore/content/home", 
        LongHelp = "This method tries to fix the bad html that we commonly see in our sitecore instance", 
    ...
    public class FixHTML : IFunction
    {
        public object Invoke(FunctionArgs args)
        {
            ...
            if (!string.IsNullOrEmpty(Obj))
            {
                ...
                Obj = Regex.Replace(Obj, @"(\s(class|style|id)=""[^"">]+"")", string.Empty);
